...
impl MultiUserMemoryManager {
    ...
    pub fn forget_user(&self, user_id: &str) -> Result<()> {
        // Remove from both caches - graph memory too
        self.user_memories.invalidate(user_id);
        self.graph_memories.invalidate(user_id);

        // Force cache to process pending invalidations
        self.user_memories.run_pending_tasks();
        self.graph_memories.run_pending_tasks();

        // On Windows, RocksDB/mmap file handles need time to release after Arc drop
        ...
        let user_path = self.base_path.join(user_id);
        if user_path.exists() {
            // Retry deletion with exponential backoff on Windows
            let mut attempts = 0;
            let max_attempts = 10;
            while attempts < max_attempts {
                match std::fs::remove_dir_all(&user_path) {
                    ...
                    Err(e) if attempts < max_attempts - 1 => {
                        let delay = 100 * (1 << attempts.min(4)); // 100, 200, 400, 800, 1600ms
                        tracing::debug!("Delete retry {} for {} (waiting {}ms): {}", attempts + 1, user_id, delay, e);
                        std::thread::sleep(std::time::Duration::from_millis(delay));
                        attempts += 1;
                    }
                    Err(e) => return Err(anyhow::anyhow!("Failed to delete user data after {} retries: {}", max_attempts, e)),
                }
            }
        }
    ...
    fn process_experience_into_graph(
        ...
        // Use pre-extracted entities from experience.entities if available (from batch_remember NER)
        // Only run NER if no entities were pre-extracted - avoids redundant ~100-200ms NER calls
        let extracted_entities = if !experience.entities.is_empty() {
            // Convert pre-extracted entity names to NerEntity format
            experience
                .entities
                .iter()
                .map(|name| crate::embeddings::ner::NerEntity {
                    text: name.clone(),
                    entity_type: crate::embeddings::ner::NerEntityType::Misc,
                    confidence: 0.8, // Pre-extracted entities have reasonable confidence
                    start: 0,
                    end: name.len(),
                })
                .collect()
        } else {
            // No pre-extracted entities - run NER
